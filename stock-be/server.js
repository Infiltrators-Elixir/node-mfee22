...
app.get("/api/stock/:stockId", async (req, res, next) => {
  ...
  console.log('aaaaaaaa', page);

  //取得目前的總筆數
  let [total] = await connection.execute("SELECT COUNT(*) AS total FROM stock_prices WHERE stock_id=?", [req.params.stockId]
  );
  console.log('bbbbb', total);
  total = total[0].total;
  //計算總共有幾頁
  ...
  //取得資料from mysql
  let [data] = await connection.execute("SELECT * FROM stock_prices WHERE stock_Id=? ORDER BY date LIMIT ? OFFSET ?" ,
  ...
  // 冒號後面網址
  //  上面冒號後面是什麼  這裡就可以拿到什麼 req.params.stockId
